...
class MIFB(nn.Module):
    '''(conv => BN => ReLU) * 2'''

    def __init__(self, in_ch, out_ch):
        super(MIFB, self).__init__()
        
        self.GMP_mip = nn.AdaptiveMaxPool3d( 1)
        self.GAP_mra = nn.AdaptiveAvgPool3d(1)
        ...
        #     nn.ReLU(inplace=True),
        #     nn.Conv1d(in_channels=32, out_channels=2, kernel_size=3, stride=1, padding=1),
        #     nn.ReLU(inplace=True),
        #     # nn.Sigmoid()
        # )
        '''Different testing'''
        self.conv1 = nn.Sequential(
            nn.Conv1d(in_channels=2, out_channels=2*in_ch, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=2*in_ch, out_channels=2*in_ch, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=2*in_ch, out_channels=2, kernel_size=3, stride=1, padding=1),
            # nn.Sigmoid()
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, feature_mra, feature_mip):
        
        feat_mra = self.GAP_mra(feature_mra)
        feat_mip = self.GMP_mip(feature_mip)
        
        x = torch.cat([feat_mra, feat_mip], dim=0)
        x = torch.squeeze(x, -1)
        x = torch.squeeze(x, -1)
        ...
        x = torch.unsqueeze(x,-1)
        channel_attn_mra = x[0,:,:,:,:]
        channel_attn_mip = x[1,:,:,:,:]
        # print('channel_attn_mra', channel_attn_mra.shape)
        # print('channel_attn_mip', channel_attn_mip.shape)
        # print('feat_mra', feat_mra.shape)
        # print('feat_mip', feat_mip.shape)
        feat_mra = channel_attn_mra * feature_mra + feature_mra
        feat_mip = channel_attn_mip * feature_mip + feature_mip
        fused_feat = torch.cat([feat_mra, feat_mip], dim=1)
...
class DCANet(nn.Module):
    ...
    def forward(self, x, mip):
        ...
        x = self.mifb_0(x, feature_list[0])
        ...
        x = self.mifb_1(x, feature_list[1])
        ...
        x = self.mifb_2(x, feature_list[2])
        ...
        x = self.mifb_7(x, feature_list[7])
        ...
        x = self.mifb_8(x, feature_list[8])
        x = self.tu_3(x)
        ...
        x = self.mifb_9(x, feature_list[9])
        x = self.tu_4(x)
