...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, tgt, memory,
                memory_mask: Optional[Tensor] = None,
                # memory_mask_mip: Optional[Tensor] = None,
                memory_key_padding_mask: Optional[Tensor] = None,
                pos: Optional[Tensor] = None,
                query_pos: Optional[Tensor] = None):
...
class MultiScaleMaskedTransformerDecoder3d(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.no_max_hw_pe = no_max_hw_pe # False
        self.is_masking = is_masking    # True
        self.is_masking_argmax = is_masking_argmax  # False
        self.num_classes = num_classes  # 2
        self.mask_classification = mask_classification  # True

        # positional encoding
        N_steps = hidden_dim // 3   # 64
    ...
    def forward(self, x, mask_features, mask_features_mip, mask = None):
        ...
        for i in range(self.num_feature_levels):
            ...
            # print('image feature', src[-1].shape)
        ...
        # print('self.query_embedweight', self.query_embed.weight, self.query_embed.weight.shape)
        # print('self.query_featweight', self.query_feat.weight, self.query_feat.weight.shape)
        ...
        outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, mask_features_mip, attn_mask_target_size=size_list[0], mask_classification=self.mask_classification)
        if self.mask_classification:
        ...
        for i in range(self.num_layers):
            ...
            # attn_mask_mip[torch.where(attn_mask_mip.sum(-1) == attn_mask_mip.shape[-1])] = False
            ...
            outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, mask_features_mip, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels], mask_classification=self.mask_classification)
            if self.mask_classification:
                predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
    ...
    def forward_prediction_heads(self, output, mask_features, mask_features_mip, attn_mask_target_size, mask_classification=True):
        ...
        outputs_mask_mip = torch.einsum("bqc,bcdhw->bqdhw", mask_embed, mask_features_mip)
        ...
        # print('attn_mask.shape', attn_mask.shape, torch.unique(attn_mask))
        
        attn_mask_mip = F.interpolate(outputs_mask_mip, size=attn_mask_target_size, mode="trilinear", align_corners=False) 
        attn_mask_mip = (attn_mask_mip.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1) < 0.5).bool()
        # print('attn_mask_mip.shape', attn_mask_mip.shape, torch.unique(attn_mask_mip))

        attn_mask_mip = attn_mask_mip.detach()
        attn_mask = torch.logical_or(attn_mask, attn_mask_mip)

        return outputs_class, outputs_mask, attn_mask#, attn_mask_mip
